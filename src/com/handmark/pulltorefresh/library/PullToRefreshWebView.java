...
public class PullToRefreshWebView extends PullToRefreshBase<WebView> {
	...
	protected boolean isReadyForPullEnd() {
		...
		if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
			return mRefreshableView.getScrollY() + 10 >= (exactContentHeight - mRefreshableView.getHeight());
		} else {
	...
	final class InternalWebViewSDK9 extends WebView {
		...
		private int getScrollRange() {
			if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
				return (int) Math.max(0, FloatMath.floor(mRefreshableView.getContentHeight() * mRefreshableView.getScale()) - (getHeight() - getPaddingBottom() - getPaddingTop()) - 10);
			} else {
